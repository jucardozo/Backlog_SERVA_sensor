...
#include "sensor_communication_New.h"
...
int8_t wakeup_acc(void){
    ...
    cmd_data[1] =0x40;  //   0+acc_mode[100]=continuous operation mode with reduced current; 0+acc_avg_num[000]=No averaging; pass sample without filtering
...
{
    I2C_Master_ReadReg(ACC_SLAVE_ADDR, ACC_DATAX_REG, 6);   //read to result register
    _delay_cycles(10000);
