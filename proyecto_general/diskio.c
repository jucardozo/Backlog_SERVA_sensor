...
// Define Functions
#define SMCLK 1 //MCLK = SMCLK = DCO ~1MHz
#define DLY_US(n)       __delay_cycles(n * (SMCLK / 1000000))   //Delay n microseconds
...
#define CMD41    (0x40+41)    	// SEND_OP_COND (ACMD) INITIALIZE THE CARD
#define CMD55    (0x40+55)    	// APP_CMD
#define CMD58    (0x40+58)    	// READ_OCR

// Peripheral definitions for MSP430FR2433 board

/*******************************************************
 *  SPI - eUSCI A0 - SD CARD
 ********************************************************/
#define SPI_SDCARD_SIMO                     BIT4    //P1.4
#define SPI_SDCARD_SOMI                     BIT5    //P1.5
#define SPI_SDCARD_CLK                      BIT6    //P1.6
...
/*
            ...
            P1SEL0 |= BIT4 | BIT5 | BIT6 | BIT7;      // set 4-SPI pin as second function x
        ...
        UCMODE_0 |  // 3-pin SPI mode (without slave transmission control a.k.a. UCxSTE pin)
                ...
                UCSSEL__SMCLK |  // eUSCI clock source = SMCLK
  ...
  */
    ...
    UCA0BR0 = 4;                             //Initial SPI clock must be <400kHz
...
static void set_max_speed(void) {
    UCA0CTLW0 |= UCSWRST;                           //Put state machine in reset
    UCA0BR0 = 0;                                  //f_UCxCLK = 1MHz/(0+1) = 1MHz
    UCA0BR1 = 0;
    UCA0CTLW0 &= ~UCSWRST;                          //Release USCI state machine
}                                   // TODO: Check speeds
